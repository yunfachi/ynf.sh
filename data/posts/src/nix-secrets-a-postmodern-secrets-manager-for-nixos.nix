{ slib, ... }:
{
  ...
  content = ''
    ...
    To start using it, you need add it to your flake inputs (sorry, there is no non-flake support at the moment, but it's planed fo the future):
      ...
      # configuration.nix
      {
        security.nix-secrets = {
          ...
          recipientAliases = {
            ...
            desktops = [ "pc" "laptop" ]; # Yep, you can reference aliases too
          };
    ...
    ${slib.highlightCode "nix" ''
      ...
      { config, ... }: {
        ...
        # nix-secrets will automaticaly mount the secret's content at its `path` option.
    ...
    Then, during a rebuild or boot, nix-secrets will decrypt the secrets from storage and mount them at their configured paths.
